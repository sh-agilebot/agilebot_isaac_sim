...
class AgilebotExperimental(Articulation):
    """Agilebot robot controller with inverse kinematics and gripper control.

    This class inherits from Articulation and provides high-level control commands for the Agilebot robot,
    ...
    def __init__(
        ...
        """Initialize the Agilebot controller.
        ...
        """
        if create_robot:
            # Load Agilebot robot from USD asset with gripper variant if requested
            variants = [("Gripper", "robotiq_2f_140")] if attach_gripper else [("Gripper", "None")]
            robot_prim = stage_utils.add_reference_to_stage(
                usd_path=get_assets_root_path() + "/Isaac/Robots/Agilebot/agilebot.usd",
                path=robot_path,
                variants=variants,
            )
        ...
        if create_robot:
            ...
            # Joint positions: [joint1, joint2, joint3, joint4, joint5, joint6, finger_joint]
            # This pose positions the robot in a good working configuration
            default_joint_positions = [0.0, -0.5, 0.5, -1.0, 0.0, 0.0]
            if attach_gripper:
                default_joint_positions.append(0.0)
    ...
    def set_end_effector_pose(
        ...
        jacobian_end_effector = jacobian_matrices[:, self.end_effector_link_index - 1, :, :6]  # Agilebot has 6 DOF
        ...
        # Set DOF targets for arm only (joints 0-5 for Agilebot)
    ...
    def open_gripper(self) -> None:
        ...
        if self._attach_gripper:
            self.set_dof_position_targets(self.gripper_open_position, dof_indices=[6])

    def close_gripper(self) -> None:
        """Close the gripper to the default closed position."""
        if self._attach_gripper:
            self.set_dof_position_targets(self.gripper_closed_position, dof_indices=[6])

    def set_gripper_position(self, position: np.ndarray) -> None:
        """Set gripper to a specific position.

        Args:
            position: Gripper position where 0.0 is open and 40.0 is closed.
        """
        if self._attach_gripper:
            if position.ndim == 1:
                position = position.reshape(1, -1)
    ...
    def reset_to_default_pose(self) -> None:
        ...
        # Base arm positions (always 6 DOFs for Agilebot)
        arm_positions = [0.0, -0.5, 0.5, -1.0, 0.0, 0.0]
