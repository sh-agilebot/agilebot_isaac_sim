...
DEFAULT_URDF_PATH = Path("urdf/gbt_c5a.urdf")
DEFAULT_STAGE_PATH = Path("urdf/gbt_c5a.usd")
DEFAULT_STAGE_NAME = DEFAULT_STAGE_PATH.name
DEFAULT_PHYSICS_STAGE_NAME = "gbt_c5a_physics.usd"
...
DEFAULT_CAMERA_PRIM_CANDIDATES = (
    "/GBT_C5A/camera_mount_link/camera_link",
    "/World/GBT_C5A/camera_mount_link/camera_link",
)
DEFAULT_ROBOT_ROOT_NAME = "GBT_C5A"
...
DEFAULT_FINGER_BIND_TARGET_CANDIDATES = {
    "left": (
        ...
        "/GBT_C5A/left_inner_finger/collisions/mesh_1/box",
        "/GBT_C5A/left_inner_finger/collisions/robotiq_arg2f_140_inner_finger/mesh",
    ),
    "right": (
        "/colliders/right_inner_finger/mesh_1/box",
        "/colliders/right_inner_finger/robotiq_arg2f_140_inner_finger/mesh",
        "/GBT_C5A/right_inner_finger/collisions/mesh_1/box",
        "/GBT_C5A/right_inner_finger/collisions/robotiq_arg2f_140_inner_finger/mesh",
    ),
}
...
def resolve_camera_prim_path(stage) -> str:
    ...
    for prim_path in matches:
        if prim_path.startswith("/GBT_C5A/"):
            return prim_path
    return matches[0]
...
def run_postprocess_actions(stage_path: Path, urdf_path: Path, physics_stage_path: Path | None, args: argparse.Namespace) -> None:
    if not args.skip_camera:
        ...
        print("Skipping camera reference addition", flush=True)
...
def add_postprocess_option_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--skip-camera",
        action="store_true",
        help="Do not add the camera reference or modify its RectLight.",
    )
